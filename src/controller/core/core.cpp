...
{
    for (int i = 0; i < m_albumModel->rowCount() - 1; i++) {
...
void Core::openImage(QString fullPath)
{
    ...
    MatAndFileinfo maf = File::loadImage(fullPath, m_nowRealFormat);
...
void Core::showImage(const QPixmap &pix)
{
    //对于动图在静止无操作时，不多次进行触发
    if (!pix.isNull()) {
        if ((m_info.suffix().toLower() == "gif" || m_info.suffix().toLower() == "apng")) {
...
void Core::changeOpenIcon(QString theme)
{
    ...
    if (m_item0 == nullptr) {
        ...
        m_albumModel->insertRow(m_allImageList.length(), m_item0); //插入model
                                                                   //        m_albumModel->setItem(0, 0, m_item0);
    }
...
void Core::changeImageFromClick(QModelIndex modelIndex)
{
    ...
    //判断是按钮点击，则打开图片
    if (modelIndex.row() == m_allImageList.length()) {
...
QString Core::nextImagePath(const QString &oldPath)
{
    ...
    //最后一张切下一张时，回到队列开头
    item = dynamic_cast<MyStandardItem *>(m_albumModel->item(m_albumModel->rowCount() - 2));
    if (item->getPath() == oldPath) {
...
void Core::creatModule(const QString &path, QStringList list)
{
    ...
    for (QString &filename : list) {
        ...
        if ("lingmo-dark" == m_theme || "lingmo-black" == m_theme) {
            item->setBackground(QBrush(Qt::black));
        } else {
            item->setBackground(QBrush(Qt::white));
        }
...
void Core::reloadImage(QModelIndex modelIndex)
{
    ...
    if (endItem > m_allImageList.length() - 1) {
        endItem = m_allImageList.length() - 1;
    }
