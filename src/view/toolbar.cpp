...
ToolBar::ToolBar(QWidget *parent) : QWidget(parent)
{
    ...
    // OCR
    m_ocr = new kdk::KBorderlessButton(this);

    m_ocr->setFocusPolicy(Qt::NoFocus);
    m_ocr->setToolTip(tr("OCR"));
    m_ocr->setAttribute(Qt::WA_TranslucentBackground);
...
void ToolBar::initControlQss()
{
    ...
    if (platForm.contains(Variable::platForm)) {
        ...
        m_toolLayout->setSpacing(20);
        m_toolLayout->addWidget(m_delImage, 0, Qt::AlignCenter);
        m_toolLayout->setContentsMargins(14, 8, 18, 12);
        g_tooleWid->setLayout(m_toolLayout);
        g_tooleWid->move(4, 4);
...
void ToolBar::initConnect()
{
    ...
    connect(m_ocr, &kdk::KBorderlessButton::clicked, this, &ToolBar::getText);
...
void ToolBar::updateCheckBtnQss()
{
    ...
    m_ocr->setIcon(QIcon(":/res/res/sp1/ocr.svg"));
