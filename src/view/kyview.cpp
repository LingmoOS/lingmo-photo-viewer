...
void KyView::naviChange()
{
    ...
    if (m_sideBar->isHidden()) {
        if (m_brushSettings->isHidden()) {
            ...
            m_navigator->move(this->width() - 7 - m_navigator->width(),
                              this->height() - 110 - m_navigator->height() - 15);
        }
    } else {
        m_navigator->move(this->width() - 7 - m_navigator->width(),
                          this->height() - toolbarHeightValue - m_sideBar->height() - 4 - 2 - m_navigator->height());
    }
}
//信息栏随顶栏位置变化而变化
...
{
    if (m_sideBar != nullptr && m_sideBar->isHidden()) {
    ...
    if (m_titlebar != nullptr && m_sideBar != nullptr && m_toolbar != nullptr) {
        if (Variable::g_mainlineVersion) {
            Variable::g_acturalWidHeight = this->height() - m_titlebar->height() - TOOLBAR_SIZE.height() - 6 - 50;
            ...
            Variable::g_acturalWidWidth = this->width();
            if (m_backWidWidth != Variable::g_acturalWidWidth) {
                m_backWidWidth = Variable::g_acturalWidWidth;
                m_sideBar->setItemNum(Variable::g_acturalWidWidth);
            }
        }
    }
    albumPosChange();
...
{
    if (m_showImageWidget == nullptr || m_toolbar == nullptr || m_sideBar == nullptr) {
        return;
    }
    if (Variable::g_mainlineVersion) {
        if (m_sideBar->isHidden()) {
        ...
        if (Variable::g_needChangeAlbumPos) {
            m_sideBar->move(0,
                            (this->height() - m_titlebar->height() - m_toolbar->height() - 6 - m_sideBar->height()) / 2
                                + m_titlebar->height());
        } else {
        ...
        if (Variable::g_needChangeAlbumPos) {
            m_sideBar->move(this->width() / 2 - m_sideBar->width() / 2, this->height() - m_sideBar->height() + 2);
        } else {
            m_sideBar->move(0, this->height() - m_sideBar->height() + 2);
        }
        if (m_sideBar->isHidden()) {
            m_toolbar->move(int((this->width() - m_toolbar->width()) / 2),
                            this->height() - m_toolbar->height() + 4 - 10);
        } else {
            m_toolbar->move(int((this->width() - m_toolbar->width()) / 2),
                            this->height() - m_toolbar->height() - m_sideBar->height() - 4);
        }
    }
    this->naviChange();
}
...
void KyView::delayHide_navi()
{
    if (displayMode == DisplayMode::OCRMode) {
        return;
    } else if (!m_titlebar->g_myEdit->isHidden()) {
        m_titlebar->show();
        this->toolBarPostionSet();
        this->toolBarHideOrShow(true);
        return;
...
void KyView::getMouseStateFromQml()
{
    ...
    if (m_toolbar->isHidden()) {
        if (displayMode == DisplayMode::NormalMode || displayMode == DisplayMode::OCRMode) {
            if (m_titlebar->isHidden()) {
                m_titlebar->raise();
                m_titlebar->show();
            }
...
void KyView::hoverChange(int y)
{
    ...
    int hoverToolbarDisBottom = this->height();
    if (m_sideBar->isHidden()) {
        hoverToolbarDisTop = this->height() - toolbarHeightValue - 10;
        hoverToolbarDisBottom = this->height();
    } else {
        hoverToolbarDisTop = this->height() - m_sideBar->height() - 4 - toolbarHeightValue;
        hoverToolbarDisBottom = this->height() - m_sideBar->height() - 4;
    }
    if (y <= titlebarHeightValue || (y >= hoverToolbarDisTop && y < hoverToolbarDisBottom)) {
...
void KyView::mouseMoveEvent(QMouseEvent *event)
{
    ...
    } else if (displayMode == DisplayMode::OCRMode) {
        this->toolBarHideOrShow(false);
        m_titlebar->show();
    }
    //    }
